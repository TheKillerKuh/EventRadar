  ...
  <section class="mt-6">
    ...
    <div class="mt-3">
      ...
      >
        ...
        <button
          @click="load"
          class="ml-auto px-3 py-1 border rounded hover:bg-gray-100 transition-colors"
        >
          Aktualisieren
        </button>
      </div>
    ...
    <div v-if="!loading && tournaments.length === 0" class="text-sm text-gray-600 mt-3">
        ...
        <div class="flex justify-between items-start gap-3">
          <div>
            ...
            <div class="text-sm text-gray-600">
              {{ formatDate(t.date) }} {{ t.time || '' }} - {{ t.location }}
            </div>
            <div
              v-if="auth.user && (auth.user.role === 'admin' || auth.user.is_admin)"
              class="text-xs text-gray-500"
            >
              Owner: {{ t.user_name || t.user_email || t.user_id || '-' }}
            </div>
          </div>
...
async function load() {
  ...
  error.value = ''
  try {
    const adminCheck = !!(auth.user && (auth.user.role === 'admin' || auth.user.is_admin))
    let url =
      adminCheck && viewMode.value === 'all'
        ? '/api/get_tournaments.php'
        : '/api/get_my_tournaments.php'
    if (adminCheck && viewMode.value === 'all' && ownerFilter.value) {
